...
SYSTEM_ALIGN_TYPE lookup_symaddr(char *name)
{
	struct symaddr *sa;
	   ...
	   char *outbuf, int outsize)
{
	...
	rel_addr = (SYSTEM_ALIGN_TYPE)reloc->r_offset;
...
int fixups(bfd *abfd, char *outbuf, int outsize)
{
	ElfW(Sym) * symtab;
	char *strtab;
	...
	for (; reloc < reloc_end; reloc = (PLTREL *)((char *)reloc + sizeof(PLTREL))) {
...
next:
	...
	if (reloc == NULL) {
		reloc = (PLTREL *)bfd_load_section(abfd, ".rela.plt",
						    &reloc_size);
	}
	if (reloc == NULL) {
		fprintf(stderr, "load error .rel.plt / .rela.plt\n");
		return 0;
	}
	reloc_end = (PLTREL *)((char *)reloc + reloc_size);
	fprintf(stderr, "reloc_size = %d\n", reloc_size);
	for (; reloc < reloc_end;
	     reloc = (PLTREL *)((char *)reloc + sizeof(PLTREL))) {
		fixup(abfd, symtab, strtab, reloc, outbuf, outsize);
	}
	return 0;
}
